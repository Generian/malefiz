...
interface UPDATE_ROLL_DICE extends BaseAction {
  updateType: "ROLL_DICE"
}

export interface UPDATE_MOVE_PIECE extends BaseAction {
...
interface UPDATE_MOVE_BLOCK extends BaseAction {
...
export const validateGameUpdate = ({ game, color, action }: GameUpdate) => {
  ...
  if (!game.gameOver) {
    ...
    if (isValid && newGame.actions) {
      newAction.createdAt = new Date().getTime()
